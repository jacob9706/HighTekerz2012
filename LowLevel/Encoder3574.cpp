 ...
 */
void Encoder3574::InitEncoder(bool reverseDirection, EncodingType encodingType)
{
 ...
 * or be double (2x) the spec'd count.
 */
Encoder3574::Encoder3574(UINT8 aModuleNumber, UINT32 aChannel,
 ...
 * or be double (2x) the spec'd count.
 */
Encoder3574::Encoder3574(UINT32 aChannel, UINT32 bChannel, bool reverseDirection, EncodingType encodingType) :
	m_encoder(NULL),
	m_counter(NULL)
{
 ...
 * or be double (2x) the spec'd count.
 */
Encoder3574::Encoder3574(DigitalSource *aSource, DigitalSource *bSource, bool reverseDirection, EncodingType encodingType) :
	m_encoder(NULL),
	m_counter(NULL)
{
	m_aSource = aSource;
 ...
 */
void Encoder3574::SetMaxPeriod(double maxPeriod)
{
	if (StatusIsFatal()) return;
	if (m_counter)
	{
 ...
 */
double Encoder3574::DecodingScaleFactor()
{
	if (StatusIsFatal()) return 0.0;
 ...
 */
double Encoder3574::GetDistance()
{
	if (StatusIsFatal()) return 0.0;
 ...
 */
void Encoder3574::SetMinRate(double minRate)
{
	if (StatusIsFatal()) return;
 ...
 */
void Encoder3574::SetDistancePerPulse(double distancePerPulse)
{
	if (StatusIsFatal()) return;
 ...
 */
void Encoder3574::SetReverseDirection(bool reverseDirection)
{
	if (StatusIsFatal()) return;
	if (m_counter)
	{
 ...
 */
void Encoder3574::SetPIDSourceParameter(PIDSourceParameter pidSource)
{
	if (StatusIsFatal()) return;
...
float Encoder3574::GetFPGAPeriod()
{
	return m_counter->GetPeriod();
}
